...
| 量 | 值 | 95% CI | 是否显著 ≠ 0 |
| --- | --- | --- | --- |
| $b_{ghost}$（原始响应） | -0.4441 | [-0.6979, -0.1897] | **是** |
| $b_{occ}$（遮挡后残余） | -0.2703 | [-0.5226, -0.0273] | **是** |
| $b_{ctrl}$（涂别处） | -0.4334 | [-0.6755, -0.1873] | **是** |

| 量 | 值 |
| --- | --- |
| **必要性比 $R$（主读数）** | **+0.461 [0.166, 0.789]** |
| 对照臂 $R_{ctrl}$（应接近 0） | +0.155 [-0.048, 0.399] |
| 过门槛事件数 | 273 |
| **判定** | **不可估：必要性比的 scene 级 CI 跨 0.5** |
