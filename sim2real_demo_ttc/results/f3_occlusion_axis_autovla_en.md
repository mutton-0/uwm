# F-3 (occlusion necessity test): readout for AutoVLA
...
**Table 1. Four-arm readouts (scene-level bootstrap). 119 events (**cost-capped for a slow model**: only the first 120 events).**

| Quantity | Value | 95% CI | Significantly ≠ 0? |
| --- | --- | --- | --- |
| $b_{ghost}$ (original response) | -0.0551 | [-0.2194, 0.0277] | no |
| $b_{occ}$ (residual after occlusion) | -0.0383 | [-0.2065, 0.0608] | no |
| $b_{ctrl}$ (patch elsewhere) | -0.0590 | [-0.2213, 0.0223] | no |

| Quantity | Value |
| --- | --- |
| **Necessity ratio $R$ (primary)** | **-0.590 [-1.607, 0.033]** |
| Control arm $R_{ctrl}$ (should be ≈ 0) | +0.055 [-0.098, 0.213] |
| Events past the gate | 73 |
