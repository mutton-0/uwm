...
**Table 1. 四臂读数（scene 级 bootstrap）。事件数 119（**慢模型控成本**：只跑前 120 个事件）。**

| 量 | 值 | 95% CI | 是否显著 ≠ 0 |
| --- | --- | --- | --- |
| $b_{ghost}$（原始响应） | -0.0551 | [-0.2194, 0.0277] | 否 |
| $b_{occ}$（遮挡后残余） | -0.0383 | [-0.2065, 0.0608] | 否 |
| $b_{ctrl}$（涂别处） | -0.0590 | [-0.2213, 0.0223] | 否 |

| 量 | 值 |
| --- | --- |
| **必要性比 $R$（主读数）** | **-0.590 [-1.607, 0.033]** |
| 对照臂 $R_{ctrl}$（应接近 0） | +0.055 [-0.098, 0.213] |
| 过门槛事件数 | 73 |
