...
| C-hazard | **已补测**（见下节）；$C_m$ **not applicable** | 剖面呈阶跃，top-2 占比公式前提结构性不成立（§CE/A39）。改报承诺层。 |
...
补测依据：`candidate_expansion_DONE.md` 把本格列为"未测：预算"。本节补上，
口径与 DiffusionDrive / LTF / DiffusionDriveV2 逐条一致：
配对 = G1 clean↔ghost（配对真实输入互换，禁用噪声破坏）；
corruption 指标 = 连续量 $v_{plan}$；退化样本按 $|v_{clean} - v_{ghost}|$ 取前 12；
patch 范围 = **整条 prompt 的残差流**（与 DD 系换全部 320 个融合 token 同构，§CE/A32）。
新增的实现细节是把 patch 打在**预填充（prefill）**前向上，
...
| 量 | 值 | 判据 | 结论 |
| --- | --- | --- | --- |
| patch-ALL recovery（充分割集自检） | **+1.000**（中位数 +1.000） | 须落在 [0.7, 1.3] | **通过** |
| 恢复剖面 | L0–L20 全部 ≈ 1.0；L21 起衰减（0.84 → 0.77 → 0.56 → 0.37 → 0.10 → **0.00**@L35） | —— | **阶跃/级联** |
| Spearman(层号, recovery) | **−0.859** | $\rho < -0.7$ ⇒ 级联 ⇒ $C_m$ 公式不适用 | **$C_m$ not applicable** |
| $C_m$（名义值，仅供核查） | 0.079 [0.069, 0.094]（36 层弥散基线 0.056） | —— | **不作为判据** |
| **承诺层**（mean recovery ≥ 0.9 的最深层） | **L20 / 36（深度 0.58）** | 阶跃剖面下的替代读数 | 决策在网络中段之后才被定死 |
| 责任层众数 / 归一化熵 | L0 / 0.158 | —— | 与阶跃形态一致 |

**判定：不可估（公式前提不成立：剖面呈阶跃）** —— 与 SimLingo 的 C-domain 级联同一处理，
不是为 AutoVLA 新设的标准。

### 为什么是阶跃，以及为什么这不是"没测出来"

patch 第 $L$ 层的输出之后，**比 $L$ 更深的每一层都由 clean 侧重算**。
纯自回归 transformer 栈里残差流是唯一通路，因此 patch 任何早期层都足以让行为完全回到 clean。
这是架构决定的，不是仪器失灵——patch-ALL 自检以 +1.000 通过即为证据。

**换 patch 范围绕不开**：另跑一次只换图像 token 段（`--tokens image`），
剖面仍在 L0–L21 饱和于 1.0，Spearman $-0.876$。故这不是范围选得不好。

因此本报告给出一条比"AutoVLA 的 C 是多少"更有价值的结论：
**$C_m$ 在 TransFuser 系与 VLA 栈之间不可比**，
而 DD/LTF 在 L6 的那个内部峰是 **TransFuser 逐级重注入设计的属性**（§CE/A39）。

### 必须并列报告的一处样本限制

选中的 12 个事件里，$|v_{clean} - v_{ghost}|$ 的**中位数只有 0.267 m/s**（范围 0.199 ~ 7.940）。
这与 F① 的结论自洽——AutoVLA 的规划速度对"目标出现"整体上没有可检测的响应，
因此本来就没有多少大 gap 的事件可选。后果是 recovery 的分母很小，
读数高度量化（实测只取到 1.000 / 0.917 / 0.000 等少数值）。
**承诺层 L20 的定位应读到"网络中段"这一粒度，不应读到具体层号。**
