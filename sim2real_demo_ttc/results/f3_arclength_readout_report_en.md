...
| **A lateral readout** (trajectory lateral offset / minimum lateral clearance to the entity) | not done | this round shows $L$ is speed-dominated (\|ρ\| up to 0.73); a quantity genuinely orthogonal to speed must be lateral. **Should be pre-registered before running**, and **must first satisfy the design constraint of §FE/A69**: the longitudinal/lateral decomposition must be taken relative to **the ego's heading at the queried frame**, never a fixed world axis — otherwise, when the ego turns, the definitions of "forward" and "lateral" drift between frames and reintroduce exactly the geometric confound just eliminated. (This project's stored `traj_*` are already in that frame's ego coordinates, so dimensions 1/2 can be used directly; do **not** transform to world. Within-frame contrasts are safe by construction; the cross-frame $b_{ghost}$ is the risk, and each frame's lateral component must be taken in its own ego basis, with the distribution of the inter-frame heading difference reported as the residual bound.) |
