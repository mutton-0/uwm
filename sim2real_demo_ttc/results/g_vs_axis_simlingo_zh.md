...
**特征**：InternVL2-1B 的 vision token（dynamic_preprocess tile 网格，每 tile 16×16），取第 12 层的 **token 级（未池化）**表征。
...
| 臂 | mIoU | 95% CI | 含义 |
| --- | --- | --- | --- |
| **trained** | **0.3963** | [0.3866, 0.4056] | 训练好的模型 |
| random_init | 0.3691 | [0.3544, 0.3831] | 同架构随机初始化（control task 地板） |
| position_only | 0.3564 | [0.3436, 0.3697] | 只用 token 坐标（空间先验地板） |

| 量 | 值 |
| --- | --- |
| token 数 / 特征维 / scene 数 | 148992 / 896 / 151 |
| object token 占比 | 0.232 |
| **selectivity（主读数）** | **+0.0273 [0.0128, 0.0424]** |
| **判定** | **PASS：表征里确有物体性信息（selectivity 的 scene 级 CI 下界 > 0）** |
