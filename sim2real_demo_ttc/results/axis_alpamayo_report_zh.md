...
**4. F② 未测，C-hazard 已补测（见下节），C-domain 不适用。**
...
C-domain：域配对语料每个时刻只有单帧，而 Alpamayo 要多帧 ⇒ **not applicable（语料侧缺失，§CE/A36）**。
C-hazard：已补测，**判不可估（剖面呈阶跃，$C_m$ 公式前提结构性不成立）**，详见下节。
...
补测依据：`candidate_expansion_DONE.md` 把本格列为"未测：预算"。口径与其余四个候选逐条一致：
配对 = G1 clean↔ghost；corruption = 配对真实输入互换；指标 = 连续量 $v_{plan}$；
退化样本按 $|v_{clean} - v_{ghost}|$ 取前 12；patch 范围 = 整条 prompt 的残差流（prefill 前向）。

**Table 3. C-hazard readout for Alpamayo-R1. 12 events / 10 scenes, 36 layers.**

| 量 | 值 | 判据 | 结论 |
| --- | --- | --- | --- |
| patch-ALL recovery（充分割集自检） | 均值 +0.965，**中位数 +1.001** | 须落在 [0.7, 1.3] | **通过** |
| 恢复剖面 | L0 0.584、L1 0.814、**L2–L16 ≈ 0.88 ~ 1.01**、L18 起降到 ≈ 0.55、末段 ≈ 0.42 ~ 0.49 | —— | **阶跃/级联** |
| Spearman(层号, recovery) | **−0.873** | $\rho < -0.7$ ⇒ 级联 ⇒ $C_m$ 公式不适用 | **$C_m$ not applicable** |
| $C_m$（名义值，仅供核查） | 0.091 [0.080, 0.106]（36 层弥散基线 0.056） | —— | **不作为判据** |
| **承诺层**（mean recovery ≥ 0.9 的最深层） | **L16 / 36（深度 0.47）** | 阶跃剖面下的替代读数 | 决策在网络中段被定死 |
| 责任层众数 / 归一化熵 | L1 / 0.520 | —— | 与阶跃形态一致 |

**判定：不可估（公式前提不成立：剖面呈阶跃）**。原因是结构性的、与 AutoVLA 同一条：
patch 第 $L$ 层后比它更深的每一层都由 clean 重算，纯 transformer 栈里残差流是唯一通路，
故早段必然饱和（§CE/A39）。

### 本候选特有的仪器自检：采样噪声地板

Alpamayo 的 rollout 是**随机采样**（$top_p$ = 0.98，$T$ = 0.6），
AutoVLA 则是 $top_k$ = 1 的贪心解码、等价确定性。这一差别必须当场量化，否则 recovery 无法解读。

同一输入换 5 个 seed，$v_{plan}$ 的实测值为
clean [9.719, 9.315, 8.764, 9.343, 9.550]、ghost [9.201, 8.617, 9.347, 9.749, 10.132]，
**pooled sd = 0.430**，达到 clean−ghost 中位 gap（1.303）的 **0.330 倍**。

本协议把两条件与全部 patch 运行**锁在同一个 seed 上**，采样噪声因此是共模的、基本抵消——
patch-ALL 实测中位数 +1.001（而不是一个漂移值）即为直接证据。
故这条地板是**保守上界**而非实际噪声。但它足以支撑两条限定：

1. 剖面上 0.88 与 1.01 之间的起伏**不可解读**；
2. **承诺层 L16 的定位精度只到"中段"这一粒度**，不应读到具体层号。

判定门已写进脚本：若 sd 达到中位 gap 的 0.5 倍即直接判不可估（本次 0.330，未触发）。

### 一处记为观察而非结论的现象

Alpamayo 的 L0 / L1 recovery 只有 0.584 / 0.814，**没有饱和**，而 AutoVLA 在 L0 即为 1.000。
一个可能的解释是：patch 第 $L$ 层的**输出**时，该层自己的 KV 仍是 ghost 侧的
（注意力发生在被 patch 的输出之前）；AutoVLA 早期层的自身 KV 贡献可忽略，Alpamayo 的不可忽略。
本轮 12 个事件的样本量与上述采样噪声都不足以把这条坐实，**记为观察，不作结论**。
