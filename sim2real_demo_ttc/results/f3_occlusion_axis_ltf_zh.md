...
| 量 | 值 | 95% CI | 是否显著 ≠ 0 |
| --- | --- | --- | --- |
| $b_{ghost}$（原始响应） | -0.0245 | [-0.0384, -0.0102] | **是** |
| $b_{occ}$（遮挡后残余） | -0.0170 | [-0.0303, -0.0032] | **是** |
| $b_{ctrl}$（涂别处） | -0.0211 | [-0.0336, -0.0075] | **是** |

| 量 | 值 |
| --- | --- |
| **必要性比 $R$（主读数）** | **+0.113 [0.044, 0.191]** |
| 对照臂 $R_{ctrl}$（应接近 0） | +0.029 [-0.031, 0.081] |
| 过门槛事件数 | 211 |
| **判定** | **FAIL：遮住关键实体后动作基本不变 ⇒ 驱动动作的不是这个实体（盲目泛化签名）** |
