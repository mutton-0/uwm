# F-3（遮挡必要性检验）：DiffusionDrive 的读数
...
| 量 | 值 | 95% CI | 是否显著 ≠ 0 |
| --- | --- | --- | --- |
| $b_{ghost}$（原始响应） | -0.0289 | [-0.0859, 0.0216] | 否 |
| $b_{occ}$（遮挡后残余） | -0.0110 | [-0.0508, 0.0304] | 否 |
| $b_{ctrl}$（涂别处） | -0.0120 | [-0.0477, 0.0291] | 否 |

| 量 | 值 |
| --- | --- |
| **必要性比 $R$（主读数）** | **+0.052 [-0.086, 0.184]** |
| 对照臂 $R_{ctrl}$（应接近 0） | +0.026 [-0.13, 0.151] |
| 过门槛事件数 | 243 |
