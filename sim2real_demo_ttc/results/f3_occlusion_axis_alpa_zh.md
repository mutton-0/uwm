...
**Table 1. 四臂读数（scene 级 bootstrap）。事件数 99（**慢模型控成本**：只跑前 100 个事件）。**

| 量 | 值 | 95% CI | 是否显著 ≠ 0 |
| --- | --- | --- | --- |
| $b_{ghost}$（原始响应） | +0.0328 | [-0.1228, 0.1907] | 否 |
| $b_{occ}$（遮挡后残余） | +0.0270 | [-0.1207, 0.1832] | 否 |
| $b_{ctrl}$（涂别处） | +0.0317 | [-0.129, 0.1956] | 否 |

| 量 | 值 |
| --- | --- |
| **必要性比 $R$（主读数）** | **+0.028 [-0.127, 0.18]** |
| 对照臂 $R_{ctrl}$（应接近 0） | -0.086 [-0.296, 0.017] |
| 过门槛事件数 | 93 |
| **判定** | **不可估：**基线响应本身与 0 不可区分** ⇒ 没有可供必要性检验的响应。** |
