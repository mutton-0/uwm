...
an outcome can be right for the wrong reason. A policy that never approaches a pedestrian because it plans short trajectories scores as well as one
...
The VL-Think Suite of~\cite{kachaev2025don} probes the degree of language--image segmentation to
quantify the multi-modal understanding gap at deployment. However, it is confined to vision--language
models, does not extend to vision-only policies, and asks how a given model may be improved rather
than which model to deploy.

In the paper~\cite{zou2023repe} the authors give a method called RepE for locating and repairing the gap
...
To address this issue, we propose a \emph{check-up}:{要总结个名字 还是就叫checkup就行？}
...
\caption{\textbf{check-up frame.The check-up proceeds in three stages: constructing pixel-level paired data, distilling the causal
...
we treat the raw response to removing a pedestrian as a poor safety signal, because a plan that moves is as likely to move towards the pedestrian as away from it. Decomposing the response into magnitude, which is represented as the faithfulness direction $\hat v_F$; and the policy's own jitter under an irrelevant edit, which is the interference direction $\hat v_I$. And the separation it buys turns a two-sided quantity into a one-sided certificate: a near-zero response shows the pedestrian did not enter the decision; a large one shows nothing. A diagnosis with an explanation from inside the three BEV planners of why the pedestrian risk goes unmanaged: the pedestrian leaves a trace in the features but is not decoded, the planning grid cannot locate it, and the plan rarely reaches it.
