...
whether the plan moves when nothing requires it, and how much the irrelevant edit moves it. (d)~Read on one driving side and tested on the other, the exams yield a prognosis for the target domain: the exam that transfers reproduces the new side's keep-clear ordering ($\rho=\NumSpBeh$), where the standard scores reach at most \NumStdRhoMax, and each verdict can be priced in frames before it is used.
...
\item \emph{A priced prognosis.} Diagnosed on one driving side and tested on the other, orderings
      and verdicts transfer: the exam that transfers reproduces the new side's keep-clear ordering ($\rho=\NumSpBeh$), against at most \NumStdRhoMax{} for the standard scores, and each verdict is priced
