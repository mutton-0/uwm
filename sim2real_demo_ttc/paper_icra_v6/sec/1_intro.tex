...
exam that transfers reproduces the new side's near-miss ordering ($\rho={\NumSpBeh}$), where the standard scores reach only from $-0.54$ to $+0.49$, and each verdict can be priced in frames before it is used.
...
\item \emph{A priced prognosis.} Diagnosed on one driving side and tested on the other, orderings
      and verdicts transfer: the exam that transfers reproduces the new side's near-miss ordering ($\rho={\NumSpBeh}$), against $-0.54$ to $+0.49$ for the standard scores, and each verdict is priced
