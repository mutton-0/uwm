...
edit that carries no driving-relevant information~\eqref{eq:FI}. Five exams, each with a control and a reference threshold, are built on these paired plans and axes (\cref{tab:exams}): how far the policy plans
...
exam that transfers reproduces the new side's near-miss ordering ($\rho={\NumSpBeh}$), where the standard scores reach only $-0.54$--$+0.49$, and each verdict can be priced in frames before it is used.
...
\caption{\textbf{The Check-Up framework.} Pixel-level paired data (a) yield the two causal axes and the five exams built on them (b); diagnosed on one driving side and tested on the other, they give a priced prognosis for the target domain (c).}
%120
...
\item \emph{A priced prognosis.} Diagnosed on one driving side and tested on the other, orderings
      and verdicts transfer: the exam that transfers reproduces the new side's near-miss ordering ($\rho={\NumSpBeh}$), against $-0.54$--$+0.49$ for the standard scores, and each verdict is priced
