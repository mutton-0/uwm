...
\caption{\textbf{The check-up.} (a)~A logged frame and its two counterfactual edits, each checked by a
detector no policy shares. (b)~The same policy is queried on all three inputs; the three plans are read
against the pedestrian's logged future through the clearance $c_t$. (c)~Four readouts taken on the
three plans over the common 2.5\,s horizon combine into the five exams of \cref{tab:exams}.}
\label{fig:method}
\end{figure*}
