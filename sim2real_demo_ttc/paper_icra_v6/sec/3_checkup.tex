...
night scene, and we refer to it as the \emph{night-style perturbation}. Two levels are used: $\mathcal{E}_D$, the milder \emph{dusk} $(\gamma,g,s,\sigma)=(1.45,0.70,0.80,2/255)$ with a
...
\label{eq:align}
\end{equation}
A policy for which lighting is irrelevant has $\CFR\gg1$ and $\mathrm{align}\approx0$. Positive alignment means the re-lighting pushes the plan the way deleting the pedestrian does. Equation~\eqref{eq:align} reads this in plan space, and its representation-space counterpart is reported in \cref{sec:lighting}. The thresholds of \cref{tab:exams} ($\HS\ge0.15$, $\mathrm{Sc}>0$, $\SP\ge0.8$, exposure within $[0.8,1.2]$, $\CFR>1$) are reference thresholds we set, not population norms.
