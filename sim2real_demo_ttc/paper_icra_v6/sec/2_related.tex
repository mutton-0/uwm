...
reward proxies~\cite{li2024egostatus,jaeger2023hidden}, and closed-loop benchmarks~\cite{jia2024bench2drive} score a policy in simulation, whereas we read released policies on real frames. We show that the six-policy spread is carried by a single gate.
...
accuracy. A metric drop, however, does not say whether a policy became more cautious or less.
...
Counterfactual explanations edit the scene~\cite{jacob2022steex,zemni2023octet}, and CausalAgents~\cite{roelofs2022causalagents} deletes non-causal agents to test motion forecasters. Our removal edit applies the same idea at pixel level to a driving policy's input, and we verify every edit with a detector. Steering directions~\cite{zou2023repe} can show that a network encodes what it does not
