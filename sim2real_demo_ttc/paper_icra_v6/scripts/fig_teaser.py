...
G=GridSpec(1,4,figure=fig,width_ratios=[1.55,1.95,0.1,1.35],wspace=0.1,left=0.005,right=0.985,top=0.9,bottom=0.14)
gA=G[0].subgridspec(3,1,hspace=0.12); gB=G[1].subgridspec(3,3,wspace=0.12,hspace=0.3,height_ratios=[0.02,1,1])
# (a) 三张图
...
for k,m in enumerate(M):
    ...
    ax.set_xlim(-4.5,4.5); ax.set_ylim(-1,21); ax.set_xticks([]); ax.set_yticks([0,10,20] if k%3==0 else [])
    ax.tick_params(labelsize=5.5,length=2)
    ax.axvspan(-1.0,1.0,color="#eef1f6",lw=0,zorder=0)
    ax.plot(-cen[1],cen[0],marker="*",ms=7,color="#e34948",zorder=5,mec="white",mew=0.4)
    ax.plot(0,0,marker="^",ms=4,color="#0b0b0b")
    c=CARD[m].get(UID); nn=NIGHT[m].get(UID)
    ax.set_title(NAME[m],fontsize=6.3,pad=2,color="#0b0b0b")
    if not c or "actual" not in c:
        ax.text(0,10,"n/a",ha="center",color="#9a998f"); continue
    for key,ls,lw in (("clean","-",1.4),("rm",(0,(3,1.5)),1.1)):
        w=np.vstack([[0,0],np.asarray(c["actual"][key])]); ax.plot(-w[:,1],w[:,0],color=COL[m],ls=ls,lw=lw)
    if nn:
        w=np.vstack([[0,0],np.asarray(nn["night"])]); ax.plot(-w[:,1],w[:,0],color="#52514e",ls=(0,(1,1.2)),lw=1.2)
    for s_ in ("top","right"): ax.spines[s_].set_visible(False)
tb=fig.add_subplot(gB[0,:]); tb.axis("off"); tb.set_title("(b) plans over 2.5 s: original —, removed – –, night ···",fontsize=7.5,loc="left",pad=3)
