...
def panel_extract(a,tok="34b62d7333845af3",mm="ddv2",title="(a) three queries, one scene",fs=1.0,legend=True,compact=False,ymax=None):
    ...
    a.plot(-ped[:,1],ped[:,0],color="#17233b",ls=(0,(1.3,1.3)),lw=0.9,zorder=5)
    a.plot(-ped[0,1],ped[0,0],marker="*",ms=6*fs,color="#17233b",mec="white",mew=0.4,zorder=6)
    for k,c in (("night","#8c9bab"),("rm","#5b9bd5")):     # 每 0.25 s 一根连线，长度的时间均值就是 I / F
        for i in range(0,len(TT),3):
            a.plot([-P["clean"][i,1],-P[k][i,1]],[P["clean"][i,0],P[k][i,0]],color=c,lw=0.45,alpha=0.75,zorder=2)
    for k,c,lw,z in (("night","#8c9bab",0.9,3),("rm","#5b9bd5",0.9,3),("clean","#1f4e79",1.3,4)):
    ...
    if legend:
        h=[plt.Line2D([],[],color="#1f4e79",lw=1.3,label="original $O$"),
           plt.Line2D([],[],color="#5b9bd5",lw=1.0,ls=(0,(2.6,1.6)),label="removed $R$"),
           plt.Line2D([],[],color="#8c9bab",lw=1.0,ls=(0,(2.6,1.6)),label="re-lit $N$")]
...
def panel_readings(b,title="(b) the two readings",fs=1.0,compact=False,schematic=False):
    ...
    if schematic:   # 框架图：只画判据几何，不放实测点（实测见 fig_cases e）
        b.fill_between([EPS,0.5],[0.5,0.5],[40,40],color="#e9f0f8",lw=0,zorder=0)
        b.fill_between([0.5,40],[0.5,40],[40,40],color="#e9f0f8",lw=0,zorder=0)
        ...
        b.text(0.06,0.93,"avoidance:\n$F\\geq0.5$ m, $F>I$",transform=b.transAxes,fontsize=4.8*fs,color="#1f4e79",va="top",linespacing=1.2)
    ...
    b.fill_between([EPS,0.5],[0.5,0.5],[40,40],color="#e9f0f8",lw=0,zorder=0)     # F≥0.5 且 F>I
    b.fill_between([0.5,40],[0.5,40],[40,40],color="#e9f0f8",lw=0,zorder=0)
    b.plot([EPS,40],[EPS,40],color="#6b6a62",ls=(0,(3,2)),lw=0.7,zorder=2)
    b.axhline(0.5,color="#9a998f",ls=(0,(1.6,1.6)),lw=0.7,zorder=2)
    ...
    b.text(0.055,0.93,"$F\\geq0.5$ m and $F>I$",transform=b.transAxes,fontsize=4.8*fs,color="#1f4e79",va="top")
    if HAS4:
...
def frame_extract(a,fs=1.0):
    ...
    a.axvspan(-1,1,color="#eef1f6",lw=0,zorder=0)
    a.plot(ped[:,0],ped[:,1],color="#17233b",ls=(0,(1.3,1.3)),lw=1.0,zorder=5)
    a.plot(ped[0,0],ped[0,1],marker="*",ms=7*fs,color="#17233b",mec="white",mew=0.4,zorder=6)
    for X,c,lw,ls,z in ((N,"#8c9bab",1.1,(0,(2.6,1.6)),3),(R,"#5b9bd5",1.1,(0,(2.6,1.6)),3),(O,"#1f4e79",1.6,"-",4)):
        a.plot(X[:,0],X[:,1],color=c,lw=lw,ls=ls,zorder=z)
    a.plot(0,0,marker="^",ms=6*fs,color="#0b0b0b",zorder=6)
    for X,c,lab,i,off in ((R,"#5b9bd5","$F$",-1,(0.0,-1.3)),(N,"#8c9bab","$I$",30,(-1.1,0.9))):   # F 在末端，I 在半程：都是同时刻两条规划的距离
    ...
    h=[plt.Line2D([],[],color="#1f4e79",lw=1.6,label="plan on $O$"),
       plt.Line2D([],[],color="#5b9bd5",lw=1.1,ls=(0,(2.6,1.6)),label="plan on $R$"),
       plt.Line2D([],[],color="#8c9bab",lw=1.1,ls=(0,(2.6,1.6)),label="plan on $N$"),
       plt.Line2D([],[],color="#17233b",lw=1.0,ls=(0,(1.3,1.3)),marker="*",ms=5,label="pedestrian")]
...
def frame_readings(b,fs=1.0):
    ...
    b.text(0.05,0.94,"avoidance\n$F\\geq0.5$ m, $F>I$",transform=b.transAxes,fontsize=5.0*fs,color="#1f4e79",va="top",linespacing=1.2)
