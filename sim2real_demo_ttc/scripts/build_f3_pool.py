...
man = {
 ...
 "criteria": {
   ...
   "遮挡组只含走廊内 VRU": "走廊外的行人**不遮**（它们不是这次刹车的原因），"
                           "但会留在画面里。见 results/unmasked_vru_residual.json："
                           "T2 中位仍有 1 个未遮 VRU 可见，scene-0017 三个事件各有 9-11 个。"
                           "若模型响应的是'画面里有没有行人'而非'我路径上这个'，"
                           "遮完走廊内这组后响应仍会残留 ⇒ **R 会被低估、判定偏向 FAIL**。"
                           "这是本语料的固有限制，须在报告中作为 R 的方向性偏差写明",
