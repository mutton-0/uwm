...
\input{tables/tab_geometry}

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/axes_geometry.pdf}
\caption{\textbf{Two families.} The three directions at each policy's deepest gated layer,
drawn on a half-circle with $v_{\mathrm{faith}}$ fixed at $0^\circ$. Shaded wedges are the
split-half floor---the angular precision the sample supports. $v_{\mathrm{decel}}$ falls close
to $v_{\mathrm{faith}}$, well outside the floor but far from random; $v_{\mathrm{bright}}$
crosses the $90^\circ$ line in every policy.}
\label{fig:geometry}
\end{figure}

\Cref{fig:geometry} and \Cref{tab:geometry} report the pairwise angles at each policy's
deepest gated layer.
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/deltab_box.pdf}
\caption{\textbf{Why we do not report a mean alone.} Distribution of $\Delta b$ under the
full-image night rendering, one point per event ($n{=}312$). DiffusionDrive and LTF show a
concentrated shift---narrow box, mean on top of median. SimLingo's interquartile range is three
times wider, its mean (diamond) sits well above its median (bar), and the points separate into
a group near zero and a group above $+3$; DiffusionDriveV2's box straddles zero. The effect
size $|\mu|/\sigma$ below each box is the number to read.}
\label{fig:deltabox}
\end{figure}

\paragraph{The mean is representative for only two of the four.} \Cref{fig:deltabox} and the
last two columns of \Cref{tab:deltab} are there because a mean alone would mislead. For DiffusionDrive the mean
