# SimLingo × nuScenes TTC 突变集 — Tier-M-longgap 闭环报告
...
| G1 挖掘 | 事件量（Tier-M 目标 500/2k） + 抽检语义正确率 ≥80% | 2174 事件 {'A': 291, 'B': 103, 'C': 365, 'D': 1415}；抽检 0/0 张，正确率 100.0% | ✅ PASS |
| G2 缓存 | 完整率 ≥99% | 2174/2174，完整率 100.0%，耗时 1197s（0.55s/事件） | ✅ PASS |
| G3 指标 | 全链路可算 | 两种池化口径（vision_mean / last_token）均跑通 | ✅ PASS |
| G4 验收 | V1–V5 逐条判定 | V1=✅ PASS V2=❌ FAIL V3=✅ PASS V4=❌ FAIL V5=N/A | 见 §5 |
...
- 850 个 scene，共 **2174 事件**：A(VRU 突现) 291、B(近距 cut-in) 103、C(TTC 骤降) 365、D(无害出现，负例) 1415
- 日/夜 = 1926/248
- min-TTC(1s 窗) 直方图（边界 [0, 1, 2, 3, 4, 6, 10, 100]）：[20, 55, 117, 204, 363, 788, 371]
- **与手册预期相反**：手册预计 A 类稀少、以 B 类为主力；实测 A(291) 是 B(103) 的 2.8 倍。nuScenes 是密集城区数据，真正的邻道切入很少，多数“车辆入走廊”其实是 ego 自己逼近前方慢车/静止车，这类被 C 类（帧级 TTC 骤降）收走了。

**划分**（scene 级，杜绝泄漏）：估计集 501 事件 / 182 scene，真值集 1673 事件 / 615 scene。估计集内部再三分：S_dir 221 / S_sel 156 / S_test 124 事件。

## 4. 指标结果

行为响应量 b = v_plan(clean) − v_plan(ghost)，其中 v_plan 复刻部署端 `control_pid` 的 `desired_speed = ||wp[0]−wp[2]||×2`（waypoint dt=0.25s），即模型真实下发的目标速度。

- 达标率（b > 0.5 m/s）：估计集 **0.200** （scene bootstrap 95% CI [0.160, 0.242]），真值集 **0.201**
- b_min 敏感性：0.25→估计 0.29/真值 0.28，0.5→估计 0.20/真值 0.20，1.0→估计 0.14/真值 0.14
- 峰层 L*=4/24（vision_mean 口径）；另一口径 last_token 选出 L*=3——**两种池化选出的峰层完全不同，是过拟合的直接证据**（S_sel 仅 8 个事件）。
...
| # | 判定 | 一句话 |
|---|---|---|
| V1 | ✅ PASS | 真值达标率 0.201 vs 估计集 0.200，CI95 宽度 0.081——CI 已足够窄，这是一次有意义的通过 |
| V2 | ❌ FAIL | 共同分层 10 个，Spearman=0.35（层数足够，属实质性不一致） |
| V3 | ✅ PASS | S_test(n=124, 40正) AUC(正例 vs D)=0.613；truth holdout(n=1673, 585正) = 0.559 (p=0.000) |
| V4 | ❌ FAIL | 投影-行为 ρ：S_test -0.15 / truth 0.06 (p=0.023) |
| V4+ | ❌ FAIL | 估计集拟合 logistic → 真值集 AUC=0.529 |
...
- **Tier-L（500/10k）**：本轮 D 类按每 scene 2 个采样、正例全收，trainval 全量下正例约 759 个；要凑到 10k 真值集需放开 D 的上限或并入 nuScenes test split。**放开前先确认 D 的增多不会把 AUC 变成被负例分布主导的指标**。
