...
| G2 缓存 | 完整率 ≥99% | 2174/2174，完整率 100.0%，耗时 631s（0.29s/事件） | ✅ PASS |
...
- 达标率（b > 0.5 m/s）：估计集 **0.321** （scene bootstrap 95% CI [0.279, 0.365]），真值集 **0.309**
- b_min 敏感性：0.25→估计 0.39/真值 0.37，0.5→估计 0.32/真值 0.31，1.0→估计 0.22/真值 0.21
- 峰层 L*=4/24（vision_mean 口径）；另一口径 last_token 选出 L*=10——**两种池化选出的峰层完全不同，是过拟合的直接证据**（S_sel 仅 8 个事件）。
...
| # | 判定 | 一句话 |
|---|---|---|
| V1 | ✅ PASS | 真值达标率 0.309 vs 估计集 0.321，CI95 宽度 0.086——CI 已足够窄，这是一次有意义的通过 |
| V2 | ❌ FAIL | 共同分层 10 个，Spearman=0.07（层数足够，属实质性不一致） |
| V3 | ✅ PASS | S_test(n=124, 40正) AUC(正例 vs D)=0.613；truth holdout(n=1673, 585正) = 0.559 (p=0.000) |
| V4 | ❌ FAIL | 投影-行为 ρ：S_test -0.11 / truth 0.01 (p=0.579) |
| V4+ | ❌ FAIL | 估计集拟合 logistic → 真值集 AUC=0.519 |
